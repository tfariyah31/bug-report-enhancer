...
def save_report(report: str, sources: list[dict], bug_description: str) -> Path:
    """Save report to two places:
    1. bug_report.md          — always overwritten (used by uploaders)
    2. bug_reports/<unique>.md — permanent local copy with unique name
    """
    from datetime import datetime
    import re
    ...
    full_output = header + report + source_section
 
    # 1. Always save as bug_report.md (for uploaders)
    OUTPUT_PATH.write_text(full_output, encoding="utf-8")
 
    # 2. Save unique copy in bug_reports/
    OUTPUT_DIR.mkdir(exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    # make a short slug from description
    slug = re.sub(r"[^a-z0-9]+", "_", bug_description.lower())[:40].strip("_")
    unique_path = OUTPUT_DIR / f"bug_report_{timestamp}_{slug}.md"
    unique_path.write_text(full_output, encoding="utf-8")
 
    print(f"\n✅ Bug report saved → {OUTPUT_PATH.resolve()}")
    print(f"📁 Archived copy   → {unique_path}")
    return OUTPUT_PATH
...
def handle_upload(title: str, report: str, severity: str, priority: str = "P2"):
    ...
    # ── Regression check based on destination ────────────────────────────────
    if choice in ("1", "2", "3"):
        try:
            from regression_detector import run_regression_check
            destination_map = {"1": "github", "2": "jira", "3": "both"}
            destination     = destination_map[choice]
 
            # Import bug_description from outer scope via report header
            import re
            desc_match = re.search(r"<!-- Input: (.+?) -->", report)
            bug_desc   = desc_match.group(1) if desc_match else title
 
            proceed = run_regression_check(bug_desc, destination)
            if not proceed:
                print(f"\n🚫 Upload cancelled — report saved locally at: {OUTPUT_PATH.resolve()}")
                return
        except ImportError:
            print("   ⚠️  regression_detector.py not found — skipping duplicate check")
        except Exception as e:
            print(f"   ⚠️  Regression check error: {e} — proceeding with upload")
